...

{
	...
	function DemoPalette(thisObj) {
		...
		const addHelpButton = (palette, buttonRect) => {
			const newButton = palette.add("button", buttonRect, "?")
			newButton.onClick = onHelpButtonClick
			return newButton
		}

		// Called when a button is pressed, to invoke its associated script
		const onScriptButtonClick = () => {
			const prevCurrentFolder = Folder.current
			...
			const scriptFile = new File(this.scriptFileName)
			scriptFile.open()
		...
		const addScriptButton = (palette, buttonRect, buttonLabel, buttonCurrentDirectory, buttonScriptName) => {
			const newButton = palette.add("button", buttonRect, buttonLabel)
		...
		if (isSecurityPrefSet()) {
			...
			const left_margin_width = 20, right_margin_width = 50, between_button_width = 5

			// Width of buttons depends on platform and language

			const buttonBaseWidth = system.osName.indexOf("Windows") != -1 ? 120 : 160
			const buttonWidth = app.language == Language.JAPANESE ? buttonBaseWidth + 17 : buttonBaseWidth

			var l_button_left  = left_margin_width;
			var l_button_right = l_button_left + buttonWidth;
					var r_button_left  = l_button_right + between_button_width;
					var r_button_right = r_button_left + buttonWidth;
			...
			alert ("This demo requires the scripting security preference to be set.\nGo to the \"Scripting & Expressions\" panel of your application preferences, and make sure that \"Allow Scripts to Write Files and Access Network\" is checked.", "Demo Palette");
		}
	}
